...
describe('computeImphash', () => {
  ...
  it('lowercases, strips the .dll extension and joins with commas', () => {
    ...
    expect(hash).toBe(
      digestOf('kernel32.sleep,kernel32.createfilew,user32.messageboxa'),
    );
  });
  ...
  it('strips .sys and .ocx extensions too, like pefile', () => {
    ...
    expect(
      computeImphash([{ libraryName: 'NTOSKRNL.exe', functions: ['ExAllocatePool'], iatAddresses: [] }]),
    ).toBe(digestOf('ntoskrnl.exe.exallocatepool'));

    expect(
      computeImphash([{ libraryName: 'HAL.dll', functions: ['KeStallExecutionProcessor'], iatAddresses: [] }]),
    ).toBe(digestOf('hal.kestallexecutionprocessor'));

    expect(
      computeImphash([{ libraryName: 'WDFLDR.SYS', functions: ['WdfVersionBind'], iatAddresses: [] }]),
  ...
  it('resolves known ordinals to their real names', () => {
    expect(
      computeImphash([{ libraryName: 'WS2_32.dll', functions: ['Ordinal_115', 'Ordinal_23'], iatAddresses: [] }]),
  ...
  it('resolves ordinals across the full table, not just the low ones', () => {
    ...
    for (const [library, ordinal, funcName] of cases) {
      ...
      expect(
        computeImphash([{ libraryName: library, functions: [`Ordinal_${ordinal}`], iatAddresses: [] }]),
  ...
  it('keeps wsock32 and ws2_32 apart where their ordinals disagree', () => {
    ...
    for (const [ordinal, ws2Name, wsockName] of conflicts) {
      expect(
        computeImphash([{ libraryName: 'ws2_32.dll', functions: [`Ordinal_${ordinal}`], iatAddresses: [] }]),
        `ws2_32 ordinal ${ordinal}`,
      ).toBe(digestOf(`ws2_32.${ws2Name}`));
      expect(
        computeImphash([{ libraryName: 'wsock32.dll', functions: [`Ordinal_${ordinal}`], iatAddresses: [] }]),
  ...
  it('falls back to ord<N> for an ordinal missing from the lookup table', () => {
    ...
    expect(
      computeImphash([{ libraryName: 'ws2_32.dll', functions: ['Ordinal_9999'], iatAddresses: [] }]),
  ...
  it('hashes a parsed fixture end to end', () => {
    const buf = buildMinimalPE32({
      directories: {
        imports: [
          { libraryName: 'KERNEL32.dll', functions: [{ name: 'Sleep' }, { ordinal: 42 }] },
        ],
      },
    });
