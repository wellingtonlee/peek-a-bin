...
describe("findCodeSection", () => {
  ...
  it("finds an executable section that is not named .text", () => {
    ...
    const pe = parsePE(
      buildMinimalPE32({
        sections: [section(".rdata", 0x1000, DATA_FLAGS), section("UPX1", 0x2000, CODE_FLAGS)],
      }),
    );
  ...
  it("returns undefined when no section is named .text or marked executable", () => {
    const pe = parsePE(
      buildMinimalPE32({
        sections: [section(".rdata", 0x1000, DATA_FLAGS), section(".data", 0x2000, DATA_FLAGS)],
      }),
    );
  ...
  it('is first-match-wins over section-table order, not ".text preferred"', () => {
    ...
    const pe = parsePE(
      buildMinimalPE32({
        sections: [section(".stub", 0x1000, CODE_FLAGS), section(".text", 0x2000, CODE_FLAGS)],
      }),
    );
  ...
  it("behaves identically on PE32+", () => {
    const pe = parsePE(
      buildMinimalPE64({
        sections: [section(".rdata", 0x1000, DATA_FLAGS), section(".code", 0x2000, CODE_FLAGS)],
      }),
    );
