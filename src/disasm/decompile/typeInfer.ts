...
export function meetTypes(a: DecompType, b: DecompType): DecompType {
  ...
  if (a.kind === b.kind) {
    ...
    if (a.kind === "array" && b.kind === "array") {
      return {
        kind: "array",
        element: meetTypes(a.element, b.element),
    ...
    if (expr.kind === "binary") {
      ...
      if (op === "==" || op === "!=" || SIGNED_OPS.has(op) || UNSIGNED_OPS.has(op)) {
        ...
        if (SIGNED_OPS.has(op)) {
          if (expr.left.kind === "reg")
            setType(expr.left.name, { kind: "int", size: 4, signed: true });
          if (expr.right.kind === "reg")
            setType(expr.right.name, { kind: "int", size: 4, signed: true });
        ...
        if (UNSIGNED_OPS.has(op)) {
          if (expr.left.kind === "reg")
            setType(expr.left.name, { kind: "int", size: 4, signed: false });
          if (expr.right.kind === "reg")
            setType(expr.right.name, { kind: "int", size: 4, signed: false });
          if (expr.left.kind === "var")
            setType(expr.left.name, { kind: "int", size: 4, signed: false });
          if (expr.right.kind === "var")
            setType(expr.right.name, { kind: "int", size: 4, signed: false });
          ...
          if (varName) {
            ...
            if (!existing || existing.kind === "unknown" || existing.kind === "int") {
              ...
              for (const v of values) {
                members.set(
                  v,
                  `VAL_${v < 0 ? "NEG_" + Math.abs(v).toString(16).toUpperCase() : "0x" + v.toString(16).toUpperCase()}`,
                );
              }
