...
describe('promoteVars — stack frame locals and params', () => {
  ...
  it('defaults an accessed local to unsigned', () => {
    ...
    expect(promote([assign(irReg('eax', 4), bpLocal(8))], { frame }).locals[0].type).toBe('uint32_t');
  });
  ...
  it('maps slot sizes to C types', () => {
    const sizes: [number, string][] = [
      [1, 'uint8_t'], [2, 'uint16_t'], [4, 'int32_t'], [8, 'int64_t'], [16, 'int32_t'],
    ];
    for (const [size, type] of sizes) {
      const frame = frameOf(stackVar({ name: 'var_8', offset: 8, size, key: stackVarKey('bp', -8) }));
...
describe('promoteVars — stack access matching', () => {
  ...
  it('matches [rbp - const] as a local', () => {
    expect(promote([assign(irReg('eax', 4), bpLocal(8))], { frame }).body[0])
      .toEqual(assign(irReg('eax', 4), irVar('var_8', 4)));
  });

  it('matches [rsp + const] as a local', () => {
    expect(promote([assign(irReg('eax', 4), spLocal(0x18))], { frame }).body[0])
      .toEqual(assign(irReg('eax', 4), irVar('sp_0', 4)));
  });

  it('matches a bare [rbp] dereference as offset zero', () => {
    const deref = irDeref(irReg('rbp', 8), 4);
    expect(promote([assign(irReg('eax', 4), deref)], { frame }).body[0])
      .toEqual(assign(irReg('eax', 4), irVar('at_bp', 4)));
  });
  ...
  it('uses ebp/esp in 32-bit mode and ignores the 64-bit names', () => {
    ...
    const asEbp = promote([assign(irReg('eax', 4), bpLocal(8, 4, 'ebp'))], { frame: frame32, is64: false });
  ...
  it('uses the lower x86 parameter threshold in 32-bit mode', () => {
    ...
    const fn = promote([assign(irReg('eax', 4), bpParam(8, 4, 'ebp'))], { frame: frame32, is64: false });
    expect(fn.body[0]).toEqual(assign(irReg('eax', 4), irVar('arg_0', 4)));
  });
  ...
  it('does not match a computed address that is not base+const', () => {
    ...
    expect(promote([assign(irReg('eax', 4), scaled)], { frame }).body[0])
      .toEqual(assign(irReg('eax', 4), scaled));
  });
});
...
describe('promoteVars — recursion into every construct', () => {
  ...
  it('rewrites inside binary and unary operands', () => {
    const stmt = assign(irReg('eax', 4), irBinary('+', bpLocal(8), { kind: 'unary', op: '-', operand: bpLocal(8) }));
  ...
  it('rewrites ternary and cast operands', () => {
    const t: IRExpr = { kind: 'ternary', condition: bpLocal(8), then: bpLocal(8), else: irConst(0) };
  ...
  it('rewrites field and array accesses', () => {
    const fa: IRExpr = { kind: 'field_access', base: bpLocal(8), structId: 's1', fieldOffset: 0, fieldName: 'f', size: 4 };
    const aa: IRExpr = { kind: 'array_access', base: bpLocal(8), index: bpLocal(8), elementSize: 4, size: 4 };
  ...
  it('rewrites try bodies, handlers and filter expressions', () => {
    ...
    expect(promoteOne(t)).toEqual({ kind: 'try', body: [ret(V)], handler: [ret(V)], filterExpr: V });
  });

  it('leaves statements with no expressions alone', () => {
    for (const stmt of [{ kind: 'break' as const }, { kind: 'continue' as const }, { kind: 'goto' as const, label: 'L' }]) {
...
describe('promoteVars — stores', () => {
  const frame = frameOf(stackVar({ name: 'var_8', offset: 8, size: 4, key: stackVarKey('bp', -8) }));
  ...
  it('promotes the stored value as well as the destination', () => {
    ...
    expect(out).toEqual({ kind: 'assign', dest: irVar('var_8', 4), src: irVar('var_8', 4), addr: undefined });
  });
...
describe('promoteVars — type inference', () => {
  ...
  it('does not retype parameters from access width', () => {
    const frame = frameOf(stackVar({ name: 'arg_0', offset: 0x10, size: 8, key: stackVarKey('bp', 0x10) }));
...
describe('promoteVars — type-based renaming', () => {
  ...
  it('renames references inside nested control flow', () => {
    const fn = promote([
      { kind: 'if', condition: bpLocal(8), thenBody: [ret(bpLocal(8))] },
    ], { frame: frameWith('var_8'), typeCtx: typeCtxOf({ var_8: HANDLE }) });
...
describe('promoteVars — return type', () => {
  ...
  it('finds a value return inside an if or else branch', () => {
    expect(promote([{ kind: 'if', condition: irConst(1), thenBody: [ret(irConst(0))] }]).returnType).toBe('int');
    expect(promote([{
      kind: 'if', condition: irConst(1), thenBody: [], elseBody: [ret(irConst(0))],
    }]).returnType).toBe('int');
  });

  it('finds a value return inside a while or do-while body', () => {
    expect(promote([{ kind: 'while', condition: irConst(1), body: [ret(irConst(0))] }]).returnType).toBe('int');
    expect(promote([{ kind: 'do_while', condition: irConst(1), body: [ret(irConst(0))] }]).returnType).toBe('int');
  });
