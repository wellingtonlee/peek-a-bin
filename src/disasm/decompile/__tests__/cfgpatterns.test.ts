...
import { detectShortCircuit, detectForLoop, detectMultiExitLoop, detectIfElseIfChain } from '../cfgpatterns';
  ...
  describe('rejects non-patterns', () => {
    ...
    it('returns null when the block has three successors (jump table)', () => {
      const blocks = [
        ...
        block(1, { preds: [0] }), block(2, { preds: [0] }), block(3, { preds: [0] }),
      ];
      expect(shortCircuit(blocks)).toBeNull();
    });
  ...
  describe('&& pattern', () => {
    ...
    it('caps the chain at eight conditions', () => {
      ...
      for (let i = 0; i < CHAIN; i++) {
        blocks.push(block(i, { succs: [99, i + 1], preds: i === 0 ? [] : [i - 1], end: ['je', 99] }));
      }
...
describe('detectForLoop', () => {
  ...
  const incReg = (name: string) => assign(irReg(name, 4), irBinary('+', irReg(name, 4), irConst(1)));
  ...
  it('only looks at the last statement of a body block', () => {
    ...
    const lifted = { 0: [initReg('ecx', 0)], 2: [incReg('ecx'), assign(irReg('eax', 4), irConst(0))] };
    expect(forLoop(lifted)).toBeNull();
  });
  ...
  it('takes the last assignment to the induction variable in the predecessor', () => {
    ...
    const res = forLoop({ 0: [first, assign(irReg('eax', 4), irConst(9)), second], 2: [incReg('ecx')] });
  ...
  it('concatenates every body block, stripping only the update block tail', () => {
    ...
    const res = forLoop(
      { 0: [initReg('ecx', 0)], 2: [a, incReg('ecx')], 4: [b] },
      [2, 4],
    );
...
describe('detectMultiExitLoop', () => {
  ...
  it('returns nothing when every successor stays inside the loop', () => {
    const blocks = [header, block(2, { succs: [3], preds: [1] }), block(3, { succs: [1], preds: [2] })];
