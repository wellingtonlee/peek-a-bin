...
describe('parseOperand', () => {
  ...
  it('resolves rip-relative addresses against the next instruction', () => {
    ...
    expect(parseOperand('qword ptr [rip + 0x100]', i, true, new RegState()))
      .toEqual(irDeref(irConst(0x401000 + SIZE + 0x100, 8), 8));
    expect(parseOperand('qword ptr [rip - 0x100]', i, true, new RegState()))
      .toEqual(irDeref(irConst(0x401000 + SIZE - 0x100, 8), 8));
  });
...
describe('liftBlock — data movement', () => {
  it('drops padding instructions', () => {
    expect(lift([['nop', ''], ['int3', ''], ['ud2', '']])).toEqual([]);
  });

  it('drops push and pop', () => {
    expect(lift([['push', 'rbp'], ['pop', 'rbp']])).toEqual([]);
  });

  it('drops jumps — control flow is handled by the structurer', () => {
    expect(lift([['jmp', '0x401100'], ['je', '0x401100'], ['jne', '0x401100']])).toEqual([]);
  });

  it('lifts a register move and records the definition', () => {
    const st = new RegState();
    const stmt = liftOne('mov', 'rax, 0x10', { state: st });
    expect(stmt).toEqual({ kind: 'assign', dest: irReg('rax', 8), src: irConst(0x10, 8), addr: START });
    expect(st.get('rax')).toEqual(irConst(0x10, 8));
  });

  it('substitutes a known source register value', () => {
    const stmts = lift([['mov', 'rax, 0x10'], ['mov', 'rbx, rax']]);
    expect(stmts[1]).toEqual({ kind: 'assign', dest: irReg('rbx', 8), src: irConst(0x10, 8), addr: START + SIZE });
  });
  ...
  it('falls back to raw asm for xchg with a memory operand', () => {
    expect(liftOne('xchg', 'qword ptr [rax], rbx')).toEqual({
      kind: 'raw', text: '__asm { xchg qword ptr [rax], rbx }', addr: START,
    });
  });
});
...
describe('liftBlock — arithmetic', () => {
  ...
  it('lifts arithmetic on memory to a read-modify-write store', () => {
    expect(liftOne('add', 'dword ptr [rbp - 0x4], 0x1')).toEqual({
      ...
      value: irBinary('+', irDeref(irBinary('-', irReg('rbp', 8), irConst(4, 8)), 4), irConst(1, 8)),
      size: 4,
      addr: START,
    });
  });
  ...
  it('recognises `xor reg, reg` as a zeroing idiom', () => {
    ...
    expect(liftOne('xor', 'eax, eax', { state: st })).toEqual({
      kind: 'assign', dest: irReg('eax', 4), src: irConst(0, 4), addr: START,
    });
  ...
  it('lifts two-operand imul', () => {
    expect(liftOne('imul', 'rax, rcx')).toMatchObject({
      dest: irReg('rax', 8), src: irBinary('*', irReg('rax', 8), irReg('rcx', 8)),
    });
  });

  it('lifts three-operand imul without reading the destination', () => {
    expect(liftOne('imul', 'rax, rcx, 0x4')).toMatchObject({
      dest: irReg('rax', 8), src: irBinary('*', irReg('rcx', 8), irConst(4, 8)),
    });
  });
  ...
  it('lifts inc and dec as +/- 1', () => {
    expect(liftOne('inc', 'rax')).toMatchObject({ src: irBinary('+', irReg('rax', 8), irConst(1)) });
    expect(liftOne('dec', 'rax')).toMatchObject({ src: irBinary('-', irReg('rax', 8), irConst(1)) });
  });
  ...
  it('lifts mul into a low half and a shifted high half', () => {
    ...
    expect(stmts[0]).toMatchObject({ dest: irReg('eax', 4), src: irBinary('*', irReg('eax', 4), irReg('ecx', 4)) });
    expect(stmts[1]).toMatchObject({ dest: irReg('edx', 4), src: { op: '>>', right: irConst(32) } });
  });
  ...
  it('lifts div into a quotient and a remainder over the original dividend', () => {
    ...
    expect(stmts[0]).toEqual({ kind: 'assign', dest: irReg('eax', 4), src: irBinary('/', irConst(100), irReg('ecx', 4)), addr: START });
    expect(stmts[1]).toEqual({ kind: 'assign', dest: irReg('edx', 4), src: irBinary('%', irConst(100), irReg('ecx', 4)), addr: START });
  });
  ...
  it('lifts the sign-extension idioms', () => {
    expect(liftOne('cdq', '')).toMatchObject({ dest: irReg('edx', 4), src: irBinary('>>', irReg('eax', 4), irConst(31)) });
    expect(liftOne('cqo', '')).toMatchObject({ dest: irReg('rdx', 8), src: irBinary('>>', irReg('rax', 8), irConst(63)) });
    expect(liftOne('cwd', '')).toMatchObject({ dest: irReg('dx', 2), src: irBinary('>>', irReg('ax', 2), irConst(15)) });
    expect(liftOne('cdqe', '')).toMatchObject({ dest: irReg('rax', 8), src: { kind: 'cast', type: 'int32_t' } });
    expect(liftOne('cwde', '')).toMatchObject({ dest: irReg('eax', 4), src: { kind: 'cast', type: 'int16_t' } });
    expect(liftOne('cbw', '')).toMatchObject({ dest: irReg('ax', 2), src: { kind: 'cast', type: 'int8_t' } });
  });
});
...
describe('liftBlock — flags and conditionals', () => {
  ...
  it('lifts setcc from the pending flag state', () => {
    const stmts = lift([['cmp', 'eax, 0x1'], ['sete', 'al']]);
  ...
  it('lifts cmovcc as a ternary over the old destination value', () => {
    const stmts = lift([['cmp', 'eax, 0x1'], ['cmovne', 'rbx, rcx']]);
    expect(stmts[1]).toEqual({
      kind: 'assign',
      dest: irReg('rbx', 8),
      src: {
...
describe('liftBlock — calls and returns', () => {
  const call = (opStr: string, opts: LiftOpts = {}) => liftOne('call', opStr, opts) as IRStmt & { call: { target: string; display?: string; args: IRExpr[] } };
  ...
  it('falls back to the instruction comment for an import', () => {
    const stmts = liftBlock(
      { id: 0, startAddr: START, endAddr: START + SIZE, succs: [], preds: [], insns: [
        { ...insn('call', 'qword ptr [rax + 0x18]'), comment: 'ws2_32.dll!connect' },
      ] },
      new RegState(), true, new Map(), new Map(), new Map(),
    );
  ...
  it('records the return register as the call result', () => {
    ...
    expect(lift([['call', '0x402000']], { is64: false })[0]).toMatchObject({ resultDest: irReg('eax', 4) });
  });
  ...
  it('collects x64 arguments from the fastcall registers', () => {
    ...
    expect((stmts[2] as { call: { args: IRExpr[] } }).call.args).toEqual([irConst(1, 8), irConst(2, 8)]);
  });

  it('stops collecting x64 arguments at the first unset register', () => {
    const stmts = lift([['mov', 'rdx, 0x2'], ['call', '0x402000']]);
  ...
  it('collects x86 arguments from the pushes before the call', () => {
    ...
    expect((stmts[0] as { call: { args: IRExpr[] } }).call.args).toEqual([irConst(1, 4), irConst(2, 4)]);
  });
  ...
  it('makes the call result available as the return register', () => {
    const stmts = lift([['call', '0x402000'], ['ret', '']]);
    expect(stmts[1]).toMatchObject({ kind: 'return', value: { kind: 'call', target: 'sub_402000' } });
  });

  it('returns the tracked value of the return register', () => {
    expect(lift([['mov', 'rax, 0x7'], ['ret', '']])[1]).toEqual({
      kind: 'return', value: irConst(7, 8), addr: START + SIZE,
    });
  });
  ...
  it('keeps a stale definition across a pop of the same register', () => {
    const stmts = lift([['mov', 'rax, 0x5'], ['pop', 'rax'], ['ret', '']]);
...
describe('liftBlock — string, FPU and SSE', () => {
  ...
  it('does not recognise the prefix when it is part of the mnemonic', () => {
    expect(liftOne('rep movsb', '')).toEqual({
      kind: 'raw', text: '__asm { rep movsb  }', addr: START,
    });
  });

  it('lifts fld and fstp through the x87 stack top', () => {
    const st = new RegState();
    expect(liftOne('fld', 'dword ptr [rbp - 0x4]', { state: st })).toMatchObject({ dest: irReg('st0', 10) });
  ...
  it('stores the x87 stack top to memory', () => {
    ...
    expect(liftOne('fstp', 'dword ptr [rbp - 0x4]', { state: st })).toMatchObject({
      kind: 'store', value: irConst(1), size: 4,
    });
  });
  ...
  it('lifts scalar SSE moves and arithmetic', () => {
    expect(liftOne('movss', 'xmm0, dword ptr [rax]')).toMatchObject({
      kind: 'assign', dest: irReg('xmm0', 16),
    });
  ...
  it('lifts an SSE move to memory as a store', () => {
    expect(liftOne('movsd', 'qword ptr [rbp - 0x8], xmm0')).toMatchObject({ kind: 'store', size: 8 });
  });

  it('lifts an SSE comparison to an eflags definition', () => {
    const st = new RegState();
    expect(liftOne('comisd', 'xmm0, xmm1', { state: st })).toMatchObject({ dest: irReg('eflags', 4) });
...
describe('liftBlock — fallback', () => {
  it('emits inline asm for an unmodelled instruction', () => {
    expect(liftOne('vpxor', 'ymm0, ymm0, ymm0')).toEqual({
      kind: 'raw', text: '__asm { vpxor ymm0, ymm0, ymm0 }', addr: START,
    });
  });
  ...
  it('keeps the source address on every statement', () => {
    const stmts = lift([['mov', 'rax, 0x1'], ['add', 'rax, 0x1'], ['ret', '']]);
    expect(stmts.map(s => (s as { addr?: number }).addr)).toEqual([START, START + SIZE, START + 2 * SIZE]);
  });
