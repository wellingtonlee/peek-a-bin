...
const LATTICE_KINDS = new Set([
  'unknown', 'int', 'float', 'ptr', 'bool', 'void',
  'struct', 'array', 'handle', 'ntstatus', 'hresult', 'enum',
]);
...
describe('API_TYPES table', () => {
  ...
  it('uses only kinds from the type lattice', () => {
    for (const [name, sig] of entries) {
      everyType(sig.returnType, t => expect(LATTICE_KINDS.has(t.kind), `${name} return`).toBe(true));
  ...
  it('never declares a void parameter', () => {
    for (const [name, sig] of entries) {
      expect(sig.params.map(p => p.kind), name).not.toContain('void');
    }
  });
  ...
  it('gives every int type a concrete power-of-two size', () => {
    for (const [name, sig] of entries) {
      const check = (t: DecompType) => {
        if (t.kind === 'int') expect([1, 2, 4, 8], `${name}: ${JSON.stringify(t)}`).toContain(t.size);
      };
...
describe('return type conventions', () => {
  ...
  it('matches known handle-producing APIs', () => {
    for (const name of ['CreateFileW', 'LoadLibraryA', 'GetProcessHeap', 'OpenProcess', 'CreateThread']) {
  ...
  it('takes a HANDLE as the first argument of handle-consuming APIs', () => {
    const consumers = [
      'CloseHandle', 'FindClose', 'FindNextFileW', 'ReadFile', 'WriteFile',
      'GetFileSize', 'SetFilePointer', 'GetProcAddress', 'FreeLibrary',
      'TerminateProcess', 'ResumeThread', 'SuspendThread', 'RegCloseKey',
      'ReleaseMutex', 'SetEvent', 'WaitForSingleObject', 'HeapAlloc', 'HeapFree',
      'NtClose', 'ZwClose', 'InternetCloseHandle', 'DeviceIoControl', 'CryptReleaseContext',
    ];
...
describe('ANSI/wide API pairs', () => {
  ...
  it('declares the same arity for both variants', () => {
    const mismatched = pairs.filter(([a, w]) => API_TYPES[a].params.length !== API_TYPES[w].params.length);
  ...
  it('declares matching types once char width is normalised', () => {
    ...
    for (const [a, w] of pairs) {
      ...
      if (JSON.stringify(narrowChars(sa.returnType)) !== JSON.stringify(narrowChars(sw.returnType))) {
