...
describe('matchRipOperand', () => {
  ...
  it('reports the matched span so a later hex scan can skip it', () => {
    ...
    expect('dword ptr [rip + 0x100], 0x3000'.slice(m!.index, m!.index + m!.length))
      .toBe('[rip + 0x100]');
  });
...
describe('resolveRipTarget', () => {
  ...
  it('resolves forwards against the end of the instruction', () => {
    expect(resolveRipTarget(insn(0x140001000, 'rax, qword ptr [rip + 0x2000]', 7)))
      .toBe(0x140003007);
  });

  it('resolves backwards against the end of the instruction', () => {
    expect(resolveRipTarget(insn(0x140005000, 'rcx, [rip - 0x1000]', 7)))
      .toBe(0x140004007);
  });
