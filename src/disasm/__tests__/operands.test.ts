...
import { buildIATLookup, parseOperandTargets } from "../operands";
...
describe("parseOperandTargets", () => {
  describe("direct branches", () => {
    ...
    it("treats imageBase as inclusive and imageEnd as exclusive", () => {
      expect(parseOperandTargets(insn(0x140001000, "call", "0x140000000"), BASE, END)).toHaveLength(
        1,
      );
      expect(parseOperandTargets(insn(0x140001000, "call", "0x140010000"), BASE, END)).toHaveLength(
        0,
      );
      expect(parseOperandTargets(insn(0x140001000, "call", "0x13fffffff"), BASE, END)).toHaveLength(
        0,
      );
    });
  });
  ...
  describe("absolute operands", () => {
    ...
    it("resolves an absolute memory reference", () => {
      const targets = parseOperandTargets(
        insn(0x401000, "mov", "eax, dword ptr [0x404000]"),
        B32,
        E32,
      );
    ...
    it("ignores small immediates and displacements below the image base", () => {
      ...
      expect(
        parseOperandTargets(insn(0x401000, "mov", "eax, dword ptr [ebp - 0x8]"), B32, E32),
      ).toEqual([]);
    });
    ...
    it("parses uppercase hex digits", () => {
      const targets = parseOperandTargets(
        insn(0x401000, "mov", "eax, dword ptr [0x40ABCD]"),
        B32,
        E32,
      );
