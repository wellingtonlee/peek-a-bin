...
function samplePE() {
  return parsePE(
    buildMinimalPE64({
      directories: {
        imports: [
          { libraryName: 'KERNEL32.dll', functions: [{ name: 'Sleep' }, { ordinal: 7 }] },
        ],
        exports: {
          dllName: 'sample.dll',
...
describe('resource envelope', () => {
  ...
  it.each([
    ...
    await expect(handler(new URL('pe://missing/x'), { fileId: 'missing' })).rejects.toThrow(McpError);
    await expect(handler(new URL('pe://missing/x'), { fileId: 'missing' })).rejects.toThrow(/not loaded/);
  });
});
...
describe('pe://{fileId}/functions', () => {
  ...
  it('hex-formats a tail-call target when present', async () => {
    const { session } = stubSession({
      pe: samplePE(),
      functions: [{ address: 0x140001000, name: 'thunk', size: 6, isThunk: true, tailCallTarget: 0x140002000 }],
    } as never);
    const functions = await body(captureResources(session).get('pe-functions')!, 'functions');
...
describe('pe://{fileId}/anomalies and /driver', () => {
  it('passes anomalies through verbatim', async () => {
    ...
    expect(await body(captureResources(session).get('pe-anomalies')!, 'anomalies')).toEqual(anomalies);
  });

  it('passes driver info through verbatim', async () => {
    const driverInfo = { isDriver: true, isWDM: false, kernelModules: ['ntoskrnl.exe'], kernelImportCount: 3, reasons: ['imports ntoskrnl'] };
