...
const STOPWORDS = new Set([
  'the', 'a', 'an', 'to', 'in', 'into', 'and', 'or', 'of', 'at', 'for', 'on',
  'with', 'both', 'current', 'this', 'its', 'entire', 'works',
]);
...
describe('SHORTCUT_GROUPS ⇄ docs/keyboard.md', () => {
    ...
    (_label, entry) => {
      ...
      expect(
        ...
        `"${entry.key}" is described as "${entry.action}" in SHORTCUT_GROUPS but as `
          + matching.map((r) => `"${r.description}" (docs/keyboard.md:${r.line}, ${r.section})`).join(' / ')
  ...
  it.each(
    [...docByToken.entries()].map(([token, rows]) => [token, rows[0]] as const),
  )('doc key "%s" still exists in SHORTCUT_GROUPS', (token, row) => {
