...
export function BatchRenameModal() {
  ...
  useMemo(() => {
    if (br?.status === "review" && br.results.length > 0) {
      setLocalResults(
        br.results.map((r) => ({ ...r, accepted: r.confidence >= 0.8 ? true : null })),
      );
    }
  ...
  const acceptHighConf = useCallback(() => {
    setLocalResults((prev) =>
      prev.map((r) => ({ ...r, accepted: r.confidence >= 0.8 ? true : r.accepted })),
    );
  }, []);
  ...
  // The three states below are three different dialogs, not one dialog with
  // changing contents, so each gets its own `key`. Without it React keeps the
  // Modal instance across a progress → review transition, the focused Cancel
  ...
  if (br.status === "decompiling" || br.status === "running") {
    const pct =
      br.progress.total > 0 ? Math.round((br.progress.done / br.progress.total) * 100) : 0;
    ...
    return (
      ...
      >
        ...
        <div className="text-xs text-gray-400 flex justify-between">
          <span>
            {br.progress.done} / {br.progress.total}
          </span>
          <span>{pct}%</span>
        </div>
        <button
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-gray-700 flex items-center gap-3 shrink-0">
        ...
        <button
          type="button"
          onClick={acceptAll}
          className="px-2 py-1 text-[10px] bg-green-800/50 text-green-300 rounded hover:bg-green-700/50"
        >
          Accept All
        </button>
        <button
        ...
        <button
          type="button"
          onClick={rejectAll}
          className="px-2 py-1 text-[10px] bg-red-800/50 text-red-300 rounded hover:bg-red-700/50"
        >
          Reject All
        </button>
      </div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {localResults.map((r, i) => {
              ...
              const rowBg =
                accepted === true ? "bg-green-900/10" : accepted === false ? "bg-red-900/10" : "";
              return (
                <tr key={i} className={`border-b border-gray-800/50 ${rowBg} hover:bg-gray-700/30`}>
                  ...
                  <td className="py-1.5 px-3 font-mono text-blue-300 truncate">
                    {r.suggestedName}
                  </td>
                  <td className={`py-1.5 px-3 font-mono ${confColor}`}>
                    {Math.round(r.confidence * 100)}%
                  </td>
                  <td className="py-1.5 px-3 text-gray-400 truncate max-w-xs" title={r.reasoning}>
                    {r.reasoning}
                  </td>
                </tr>
              );
