    ...
    >
      ...
      <div className="absolute top-2 right-2 z-10 flex items-center gap-1 bg-gray-900/80 px-1.5 py-0.5 rounded text-[10px] text-gray-400">
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            zoomToCenter(Math.max(0.2, zoom - 0.1));
          }}
          title="Zoom out"
        >
          −
        ...
        <input
          ...
          value={zoom}
          onChange={(e) => {
            zoomToCenter(parseFloat(e.target.value));
          }}
        ...
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            zoomToCenter(Math.min(3, zoom + 0.1));
          }}
          title="Zoom in"
        >
          +
      ...
      >
        ...
        >
          ...
          {visibleEdges.map((edge, i) => {
            ...
            if (toBlock.y <= fromBlock.y) {
              const loopX =
                ox +
                Math.max(fromBlock.x + fromBlock.w, toBlock.x + toBlock.w) +
                30 +
                Math.abs(offset);
              return (
                <path
        ...
        {/* HTML block layer */}
        {blocks
          .filter((b) => visibleBlockIds.has(b.id))
          .map((block) => (
            <CFGBlock
              key={block.id}
              block={block}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-gray-500 text-[0.75em]">{block.insns.length} insn</span>
      </button>

      {/* Instruction rows */}
      {!collapsed &&
        block.insns.map((insn) => {
          ...
          return (
            ...
            >
              ...
              <button
                ...
                onDoubleClick={(e) => {
                  e.stopPropagation();
                  onDoubleClickAddr(insn.address);
                }}
              >
              ...
              {isEditing ? (
                ...
                <span
                  className="disasm-comment ml-1 truncate max-w-[8em] text-[0.75em]"
                  title={userComment || insn.comment || ""}
                >
