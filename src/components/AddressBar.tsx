...
export function AddressBar() {
  ...
  return (
    <div className="flex items-center gap-1 px-3 py-1.5 toolbar-bg border-b border-theme text-sm">
      ...
      {/* The failed case first: the spinner below is keyed on `!disasmReady`,
          which a dead engine never clears, so it claimed the engine was still
          loading for the rest of the session while the banner above said it had
          failed (peek-a-bin-b3jn). */}
      {state.disasmFailed && (
        <span
          className="text-red-400 text-xs mr-2 flex items-center gap-1"
          title={state.disasmFailed}
        >
      ...
      {!state.disasmReady && !state.disasmFailed && (
