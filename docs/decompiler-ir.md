...
**IR** (`ir.ts`): `IRExpr` union (12 kinds: const, reg, var, binary, unary, deref, call, cast, ternary, field_access, array_access, unknown) + `IRStmt` union (18 kinds including if/while/do_while/for/switch/break/continue/phi/try/**branch**). `UnaryOp` gained `"&"` at peek-a-bin-5b6q.1 — a union MEMBER, not a kind, so it enters no dispatch census either: `promote.ts` mints it for a `<sp>/<fp> ± const` that `matchStackAccess` names, and the only sites reading the operator rather than recursing are `fold.ts`'s constant fold and double-negation rule, which name `~`/`!`/`-` explicitly, and `emit.ts`'s unary arm. `promoteVars` runs after `foldBlock` and `inferTypes`, so only `structs.ts`, `decompile/prologue.ts` and the emitter ever see one. `IRLabel` carries an optional `note` — a FIELD, not a kind, so it enters no dispatch census; `pipeline.ts`'s `annotateLabels` is the only writer and `emit.ts` the only reader, printing it on the line after the label (see the `baseGenerations` entry below for why a label no `goto` names must stay in the IR untouched).
...
`liftBlock` also clears `RegState`'s flag state on any instruction that is neither `isFlagTransparent` nor a modelled setter, which keeps `setcc`/`cmovcc` reading their own instruction's state. Since `peek-a-bin-n9cl.6` that state names a spoiled compare's **captures** rather than its operands wherever `operandCaptures` placed them — the one map every in-block flag reader (the Jcc, `setcc`, `cmovcc`) is built over, so `RegState` needs no spoil tracking of its own. That is a *different* question from ownership and the two are deliberately not merged: the SSE `comis*` forms set flags `setcc` reads, and `flagModel` classes them as a clobber, so a `comis` leaves `setcc` working and produces no branch.

**CF is also a value, and since `peek-a-bin-n9cl.6` it is an expression a `sbb`/`adc` consumes, never a statement.** `flagModel.ts` carries a second, per-flag grammar beside the whole-flags one — `CARRY_PRESERVERS`, the four setter tables, `carryEffect`, `carryOwnerBefore` over `carryScanStream` — because CF outlives the whole-flags owner (`inc`/`dec` write every arithmetic flag but CF; `sbb d, d` leaves `d = -(CF)`). `lifter.ts`'s `carryFor` asks it at each `sbb`/`adc` exactly as `branchFor` asks `blockFlagOwner` at a Jcc, spells the answer (`cmp`/`sub` → `a u< b`, `neg`/`sbb d, d` → `d != 0`, logical ops → 0, `bt` → the bit) with a spoiled setter read through the (a) captures, and substitutes it: `sbb d, d` → `-(CF)`, `sbb d, s` → `d - s - CF`, `adc d, s` → `d + s + CF`. `add`/`adc`/`sbb d, s` carry-out is refused (no wraparound model) and refusal is `raw`. `sbb`/`adc` are `RESULT_OWNERS` now, so a Jcc reads their ZF/SF — with `branchFor` refusing an owner the lifter left `raw`, because the paragraph below is exactly the defect a statement-shaped CF would reintroduce.
...
`collectCapturedCalls` and `collectDeclarations` (which replaced `collectAssignedRegs` in
`peek-a-bin-n9cl.4`), and that ordering is the whole of its safety rather than tidiness. Five things:

- **Asking both passes about the folded body is what keeps the output self-consistent.** The fold
  removes a mention of the accumulator, and the declaration block is computed from the mentions the
  reader will see: a function whose only mention of RAX was the folded pair declares no `rax` at
  all, and one that still reads it elsewhere declares it at the widest alias that survives — so
  nothing is declared that the text does not name and nothing the text names goes undeclared.
  (Historical, from the era of undeclared registers: the fold could only ever **withdraw** a
  `registerText` respelling, never add one, and moved the k8i instrument in the good direction —
  invented prelude declarations **1495/2506/2328/1451 → 1460/2456/2278/1417**, −169 corpus-wide;
  the one PE32 function where a respelling was withdrawn, `t32!sub_40E714` / `w32!sub_40CE34`, is
  decoded `00 00` padding past a `ret`.)
...
**Registers are declared variables — one C variable per canonical register per function, declared at emit** (`registerVariables`, `narrowRegisterWrite`, `registerText`, `collectDeclarations` in `emit.ts`; `peek-a-bin-n9cl.4`). The long form of the CLAUDE.md rule, with the measurements.

- **The defect.** `emitFunction` declared params, locals, `flg_` captures and `__unrecovered_N`, and nothing else: every register name, every `<reg>_<n>` split repair (`ssadestroy.ts`'s `staleName`), every `clobbered_<reg>_<n>`, `tmp_xchg`, `st0` and `stk_<addr>` reached the page undeclared. Registers were therefore free variables **one per name**, so `dx` and `edx` were two unrelated C variables for one machine register: t64's `wcslen` (`sub_14000A4AC`) assigned `edx` and tested `dx`, a loop that as C tested a variable nothing assigns (`peek-a-bin-uxm`). `registerText`'s alias search patched only narrow READS with a strictly wider *assigned* alias; the narrow-WRITE/wide-read half (`al = rcx != 0; if ((uint32_t)rax != 0)`, `w64!sub_140002190`) had no variable to write. 979/1072 functions and 7,519 (function, name) pairs carried the class at `6299113`; `preludeFor` inventing `long X;` per gcc diagnostic was the only reason `cc` was 1072/1072 clean.
- **Why declare at emit rather than promote registers to `IRVar`s per phi web (rejected).** Every downstream identity fact dispatches on `kind === "reg"`: `fold.ts`'s single-use inlining moves only register assignments and guards RSP by `canonReg`; `structs.ts`'s `canonBase`/`stackDerivedBases`/`paramIndexByBase` key `reg:rcx`; `promote.ts`'s `frameRegisterAliases` tests `e.kind === "reg"`; `structure.ts`'s `extractCondition` reads `RegState`; `emit.ts`'s `killedReg`/`collectCapturedCalls` compute call-result liveness on `canonReg` of reg nodes. Converting mid-pipeline re-teaches all of them for an identical result, and a per-web name cannot spell a PARTIAL write soundly without the reaching version, whereas one C variable per register holds the previous full value by construction.
- **The rule.** `collectDeclarations` walks the structured body (reads, assign destinations, a `call_stmt`'s `resultDest` only when `_capturedCalls` prints it, an indirect call's register held as TEXT `(*esi)` — the one mention that is not an `IRReg`, and the shape of all 16/4/4/16 names left undeclared after the first run) grouping register aliases by canonical name. `registerVariables` takes the widest alias mentioned (`aliasRank`: width first, a high byte below its low-byte sibling; `ah` beside any other alias forces at least `ax`), **caps it at `regAtSize(canon, 4)` when `!func.is64`**, and types it `sizeToType(width)` refined from `typeCtx.types.get(canon)` only when `castWidth(typeToString(t)) === width` — the `capturedOperandType` rule, which is what revives the dead `inferTypes → emit` register channel and what keeps `HANDLE`/`struct_N*` out of a declaration (so `emitsAsPointer` stays false and byte arithmetic off a register cannot be rescaled). A read of the declared name is itself; a narrower alias is `(uint16_t)edx`, a high byte `(uint8_t)(eax >> 8)` (23 mentions over 4 functions per PE32 binary, 11 over 3 per x64 — spelled rather than refused, or each would be an undeclared name); a read **wider** than the declared name — only possible past the PE32 cap — is left exactly as written and undeclared, so both `unencodableNames` and the undeclared gate see the leak. A narrow **write** is `rax = (uint32_t)e` for a 32-bit write on x64 (Intel SDM vol. 1 §3.4.1.1; the arm can only fire when the declared width is 8) and `eax = (eax & ~0xFF) | (uint8_t)e` / `~0xFFFF` / `(eax & ~0xFF00) | ((uint8_t)e << 8)` for 8/16 bits and the high byte — `~0xFF` is an `int` of −256, all-ones above bit 7 at every declared width. `narrowedValue` avoids a second cast where the text already is one (a `(uint8_t)eax` read, a lifted `movzx`, a 1/2-byte deref, a constant that fits). No compound spelling for a narrow write. Every other `IRVar` the body names — `ecx_3`, `clobbered_rcx_2` — is declared at `IRVar.size`, uninitialised.
- **`IRFunction.is64`** is set by `promoteVars` from the pipeline's own flag. It must never be inferred from the body the way `registerSpeller` infers its own: `peek-a-bin-0s6e`'s `rcx_18 = rcx` was a canonical name leaking INTO a 32-bit body, and a cap inferred from that body would read the leak as a 64-bit image.
- **Residue, counted and not declared:** `stk_<addr>` (a matched push/pop's slot), `tmp_xchg`, `st0..7` (10 bytes), `xmm`/`ymm` (16/32) — names `isKnownRegister` refuses or widths C has no integer for. 12 `stk_` mentions per PE32 binary (6 pairs), none of the others, at `2328657`. `corpus/emitAudits.ts`'s `RESIDUE_NAME` files them before `MINTED_NAME` and `build/readabilityCensus.test.ts` pins the order.
- **The gate**: `corpus.audit.ts`'s undeclared row is `register + minted === 0`, named per row first. Measured at `2328657`: register **1460/2443/2265/1417 → 0/0/0/0**, minted 6/17/17/6 → 0. **Negative control** (skip `collectDeclarations` once): 1518/2612/2419/1471 register and 0/17/17/0 minted return, the gate red naming `sub_401A4E@0x401a4e ecx (register)` first. Liveness kept: `compiled == ccSyntaxCheck.compiled`; `api + unknownTypes > 0` — **`api` is 0 on every binary**, so the assertion holds through the `unknown type name` inventions (21/281/278/20 pairs, one distinct: the handle typedef); a change that declared those types needs a *new* liveness half, not a weaker one.
- **What moved on the page** (`s29-main-2328657` → this change, `npm run corpus:compare`): emitted C changed in 247/260, 246/279, 241/275, 245/258 functions; lines 15301→16839, 15745→17932, 14189→16221, 13974→15474 — declarations 1407/2090/1937/1368, narrow-read casts 295/877/741/381, zero-extending 32-bit writes 0/1114/897/0, mask-merge writes 102/105/94/93. **Every standing gate flat**: polarity 521/576/499/445 correct with 0 inverted and the audited set unmoved, `guard lines unparsed` 0, staleGuards named 0, crossEdgeGuards 0, selfAssigns wrong/unresolved 0, lostDefs 0, popReads 0, arity over 0, `offsetof` 1.00, structOverlaps 0, unencodableNames 0/518, gcc 1072/1072, `insns covered` unchanged. `compare.mjs` flags **26/246/202/23 CHANGED guards at a shared jcc**: adjudicated over the artifacts, **0 operator, edge, kind or anchor moves**, and after stripping casts and canonicalising aliases **0 text differences** — every one is `al != 0` → `(uint8_t)eax != 0`, `ecx > 0x53` → `(int32_t)rcx > 0x53`, or the AH spelling `ah >= al` → `(uint8_t)(ax >> 8) >= (uint8_t)ax` (4 rows). Eight hand-read against `objdump -d -M intel`: t64 `sub_14000A4AC` (wcslen: `edx = (uint16_t)*…; while ((uint16_t)edx != 0)` — the tested name is now the assigned one), w64 `sub_140002190` (`setne al` → `rax = (rax & ~0xFF) | (uint8_t)(rcx != 0); if ((uint32_t)rax != 0)`, the merged value read), t32 `sub_401A4E` (three `mov dx,[eax]/test dx,dx` loops → `edx = (edx & ~0xFFFF) | *(uint16_t*)(eax); … while ((uint16_t)edx != 0)`), t32 `sub_4052AC` (`mov al,[ebp-0x512]; test al,al` → `eax = (eax & ~0xFF) | (uint8_t)var_512; if ((uint8_t)eax != 0)`), w32 `sub_406AC2` (`test al,1`/`cmp al,0xd`/`test cl,cl; js` → `((uint8_t)eax & 1)`, `(uint8_t)eax == 0xD`, `(int8_t)ecx < 0` — the signed spelling for `js`), t64 `sub_140001150` (`movzx eax,[r13]; cmp ax,[rip+…]` → `rax = (uint32_t)(uint16_t)*…; if ((uint16_t)rax != *(uint16_t*)(0x140011E0C))`), t64 `sub_140009794` (`xor eax,eax; mov r12d,eax; mov edi,eax` → `eax_3 = 0; rax = (uint32_t)eax_3;` — a zero-extension the machine performs), and t64 `sub_1400045DC` / w64 `sub_14000496C` (the phi-copy defect below, now `r9_0 = r9;` with the six stores through `r9_0`).
- **Pre-existing class measured and NOT taken on: width-blind constant/copy propagation.** `mov al, 5` beside a wider read of EAX reaches the page as `*(int32_t*)(edx) = 5` — the constant is substituted into the 32-bit read with no truncation or merge, before emit ever sees a register (100/105/94/91 sub-register writes beside a wider alias on t32/t64/w64/w32 at `2328657`). The two new `pipeline.test.ts` cases use a memory load as the narrow source for exactly this reason. Of the 394 mask-merge lines emitted, 58 (13/17/16/12) merge into a variable with no assignment above them — an incoming value, or a zeroing the propagator folded away; honest in C, and the propagation class is the thing to fix, in its own bead.
- **The one thing this broke, and the value-level repair that replaced a spelling device.** `peek-a-bin-pzws` had spelled two live ranges of one register as two names — `r9` for the entry pointer, `r9d` for the 32-bit counter — in `t64!sub_1400045DC` / `w64!sub_14000496C` (`mov rbp, r9 / mov r9d, r14d / … / mov [rbp+0x18], esi`). Under one variable per register the counter's only write, a lowered phi copy in the loop header's predecessor, prints `r9 = (uint32_t)r14d_1` above six stores through `r9`, and `corpus/staleReads.ts` — whose `writes` test now compares canonical registers, the only correct test when the identifier IS the register — went red at exactly 6 per x64 binary (`wrong-indeterminate`, 0x140004883–0x140004898). The gap was in `splitStaleReads`'s version-0 rule: `defBlocks` recorded a phi's write in the phi's **own** block, while `destroySSA` emits the copy at the end of each **predecessor** — the header does not dominate the guarded stores, the predecessor does (the same attribution `corpus/staleReads.ts` has made since `peek-a-bin-fppy`). Fixed in two halves, each with its own end-to-end case and negative control (`pipeline.test.ts`, "a phi's lowered copy is a write in the predecessor it lands in"): (a) every predecessor where a copy will be emitted — a cross-register or call-clobbered operand; a same-register operand the register still holds is skipped there as a no-op — is noted in `defBlocks`; (b) `entryState` reads a predecessor's exit through `phiCopiesOut`, so an exit that still says "entry value" hands the phi's version to its *non-phi* successors and a version-0 read on a pure bypass path is stale at all (without (b) the merge shape is repaired and the bypass shape is not — measured). Cost: exactly one `r9_0 = r9;` per affected function, 2 corpus-wide; `entry copies taken` 153→154 and 144→145. Not modelled, on the record: a copy from a stale operand's *repair* variable writes value(W) over a register holding V on the bypass edge — a lost-copy hazard independent of version 0 and of this bead. The audit's history paragraph is inverted rather than dropped: the twelve shapes the name test once excused would now be real clobbers, and are 0 because they are repaired.
- **Three text scans had to be taught the declaration block**, and `stripDeclarationBlock` (`corpus/emitAudits.ts`) is the one declaration of its shape (every line declaration-shaped AND a blank line closing it — 0 violations over 1072 functions, 93 with no block): `auditClobbered` counted `int64_t clobbered_rcx_4;` as a read (21 → 38 on each x64 binary, exactly + the 17 distinct names), `stackPointerScaffolding` counted `int64_t rsp;` as a stack-pointer read (`write and never read` 4/18/17/4 → 0), and `copyPairs` read a declaration as a statement between two others.

**A dereferenced constant is named from the deref plus the section table** (`globalAt`, `collectGlobalWidths`, `externDeclarations` in `emit.ts`; `decompile/naming.ts`; `peek-a-bin-5b6q.4`). The long form of the CLAUDE.md rule; the gotcha entry in `docs/gotchas.md` carries the measurements and the hand-readings.

- **Where the rule is asked, and why only there.** `emitExpr`'s `deref` arm and `emitStmt`'s `store` arm are the two places the IR reads or writes memory at an expression, and both hand `globalAt` the address and the width. A `const` reaching `emitExpr` on its own — as a call argument, an assignment source, a comparison operand — never reaches it, which is the whole provenance rule: the deref IS the evidence that the constant is an address of an object. `field_access`/`array_access` bases and an indexed `deref(const + idx*scale)` are not bare constants and are not named in this stage; the stride-walk shape is the register-base grouping `structs.ts` already refuses to fabricate over, and naming its base as a scalar would be the same fabrication with a `g_` on it.
- **The pre-pass exists because the first access has to know the last.** `collectGlobalWidths` runs in `emitFunction` after `foldReturnedCallResults` (the body the reader will see) and before any statement is spelled, over `walkStmts` for `deref` expressions and a `bodiesOf` walk for `store` statements (plus a `for`'s `init`/`update`, single statements `bodiesOf` deliberately does not enter). Without it the scalar-or-array decision would be taken at the first site and contradicted by the third.
- **Order of answers, and what each refusal protects.** Cookie first, because it is an identity and a section is a weaker claim; IAT slot second, refusing any width but the pointer's — a slot IS in a data section, so falling through would hand it to `g_` under the wrong name; string address third, keeping the older `*(uint8_t*)("…")`, since the literal is what the reader wants there and the two substitutions must not fight; section last. `Number.isSafeInteger(va) && va >= 0` is the guard against the `movabs` refusal's `raw` constants ever reaching a name.
- **The declaration block's order is stable by construction**: CRT-named globals in first-use order (one today), `__imp_` by name, `g_` by address — so a body change moves no unrelated `extern`. After the typedefs and before the header, `lineAddrs` undefined, so the line map never points a declaration at an instruction; `emit.test.ts` pins the order and the map.
- **`__imp_` from a thunk target is declared by `calleeText`**, the one place that sees the `(*__imp_X)` text `importThunkTransfer` minted, recorded without a `lib!func` because the thunk's own comment line already carries it. This is what moved `undeclared other` 4 → 1 on each x64 binary.
- **Two things the census cannot see** (`corpus/globals.ts`): whether `g_414620` and `g_414624` are two objects or one struct MSVC's source had — the rule names addresses, not objects — and whether `int32_t` is the declared type rather than the width the body read. And one thing it does see that the pipeline tests cannot: `.rsrc` and `.reloc` are data sections by `isDataSection`, so a deref there would be `g_X /* .rsrc */`; 0 rows on all four.
- **Refused: `.rdata` scalar folding**, and two later S candidates — SSE loads from a non-writable section printed as the float, `/* = 0x… */` annotations (never substitution) for sub-pointer widths with no relocation over the address. Both need the data windows retained in the worker, which is `peek-a-bin-9gc9`'s memory decision.
**A `GetProcAddress` result stored into a global names the global `pfn_<proc>`** (`disasm/pfnGlobals.ts`; `CalleeClobbers.pfn` in `callSummary.ts`; `NamingContext.pfn` in `decompile/naming.ts`; the `pfn` arm of `globalAt` and the `extern intptr_t` line of `externDeclarations` in `emit.ts`; `peek-a-bin-5b6q.5`). The long form of the CLAUDE.md rule; the gotcha entry carries the measurements, the controls and the hand-readings.

- **Where it sits in the pipeline: outside it.** The pre-pass runs over the whole instruction stream once per image, in the same pass as `buildCallSummaries` and `recogniseCrtIdioms` — `CallSummaryCache.forToken` for the browser, step 8c of `mcp/session.ts` for the MCP server and the harness — and its result is handed to `emitFunction` through `NamingContext`, the emitter's evidence record. No pipeline stage reads it: the IR still carries `store(const G, …)` and `deref(const G)`, and only the spelling changes. That is what makes the change runtime-inert for every stage before emission, and why the polarity census sees exactly the renamed-operand rows.
- **Order of answers in `globalAt`, extended.** Cookie, IAT slot, string address, then `pfn_`, then `g_`. `pfn_` comes before `g_` because its ground is stronger — a store instruction reconciled against every other absolute writer in the image — and after the string check for the same reason the string check precedes `g_`: a slot address that is ALSO a string address is a contradiction the emitter should not resolve by picking. It does not require `dataRangeAt` to place the address; the census asks that question instead.
- **The width rule reuses `collectGlobalWidths`.** The pre-pass computed before the body already knows every width the body reads each address at; `pfn_` is spelled only when that set is exactly `{pointer width}`. A slot also read at 4 bytes falls through to the `g_` byte array, so the two rules cannot name one address two ways in one function.
- **The declaration block's order, extended.** CRT-named globals in first-use order, `__imp_` by name, `pfn_` by address, `g_` by address. The `pfn_` comment carries the string and the slot's address — the address so the reader can find the store in the listing, and so `corpus/globals.ts` can place it against the section table without importing the emitter's rule.
- **Refused: a typed function-pointer cast, and `typeInfer` keying on the name.** Both in the gotcha entry. The second was permitted (types are not arity) and found inert: every corpus call through a slot is `call rax`/`call eax` after a null guard, so the IR's target is a register and never the slot.
- **What the census pairs.** `BinResult.pfnPrepass` is what the pass FOUND (recognised, encoded, refused by reason, from `af.calleeClobbers.pfn` — never recomputed); `corpus/globals.ts`'s `pfn*` fields are what REACHED THE PAGE (declarations, mentions, `DecodePointer` reads, indirect casts reading a `pfn_` name, slots outside a data section). `corpus.audit.ts` asserts `recognised + refused >= lookups` and `pfnDistinct === recognised` per binary — the two halves must agree — and `lookups > 0` on every binary as liveness.

**Entry registers are parameters — `rcx/rdx/r8/r9` → `arg_0..arg_3` on x64, `ecx/edx` → `arg_ecx`/`arg_edx` on x86, bound at SSA version 0** (`entryBindings.ts`, `bindEntryValues` in `ssadestroy.ts`, `dropParameterIdentities` and `fullWidthVarReads` in `promote.ts`, `paramIndexByBase` in `structs.ts`; `peek-a-bin-n9cl.5`). The long form of the CLAUDE.md rule.

- **Why version 0, and why before `splitStaleReads`.** `newVersion` starts definitions at 1, so version 0 is the one version no statement defines — the incoming value — and after `stripVersionsStmt` that fact is gone. `splitStaleReads` already owned the read-rewriting machinery (`mapReads` with its `branch` arm, `phiRepairs`), and its version-0 rule takes an entry copy `rcx_0 = rcx` where a dominating write has overwritten the register. Bound FIRST, the splitter never meets a bound read in a statement; the one place it still can is a phi operand (typed `IRReg`), which it declines (`bound` parameter), and `destroySSA` lowers the operand to `rcx = arg_0` — the third copy shape, noted in `phiCopiesOut`/`defBlocks` as a write in the predecessor exactly like the cross-register and call-clobbered shapes. Bound AFTER, the splitter would have parked the entry value in `rcx_0 = rcx`, a copy of a C variable nothing initialises once its entry value is `arg_0`.
- **What a read becomes.** `{kind:"var", name, size: <register width>}` for a read at the register's width; the SAME variable at the read's width (`size: 4` for `ecx`) for a narrower read, which `emit.ts`'s `varText` spells `(int32_t)arg_0` or `(uint32_t)arg_0` from the operation — `registerText`'s rule, so the two families look alike and, more to the point, agree about signedness. A `cast(uint32_t, var)` node was the first cut and the corpus refuted it: `test ecx, ecx / js` printed `(uint32_t)arg_0 < 0` (constantly false) at 10 guards per x64 binary, invisible to the polarity audit, which judges the operator. `(uint8_t)(arg_0 >> 8)` for a high byte, which no width expresses. No cast on a phi operand: it is minted at the canonical width (8), never narrower than the binding; on x86 the copy's destination is spelled by `registerSpeller` (`ecx`, size 4) and its source `arg_ecx` at 4.
- **The dedupe is ABI truth.** `promoteVars` builds register parameters from the same table, in argument order AHEAD of the stack slots (source order on both architectures), and where `stack.ts` already named the homed slot `arg_<n>` (it saw the spill — `inUnfilledHomeSpace`) the frame's declaration is kept in the register's position at the spill's width: one parameter, `var:arg_0` from either direction. `IRParam.register` marks a register parameter; a deduped homed slot keeps it absent, since the slot's name really is frame evidence.
- **The identity deletion and its scope.** `arg_0 = arg_0` (through casts) for a PARAMETER name only, at every depth via `rewriteBodies`. A local's or a register's identity is untouched: those are lost operands and `corpus/selfAssigns.ts`'s subject. Negative control measured: skipping the deletion files each x64 homed spill as an `openOperand` `mov` row (the store's destination is memory, not a register, so `wrong` never fires — the bead predicted the wrong column).
- **The struct half.** `paramIndexByBase` keys every `^arg_(\d+)$` parameter as `var:arg_<n>` and has no register branch; the collision logic ("home slot wins", `claimedBy`) is deleted with it because the collision cannot arise — a reused RCX is `reg:rcx`, a different key. `stackDerivedBases` reads `p.register === undefined && STACK_PARAM_RE.test(p.name)`: without the field every x64 function with a signature would have had `FRAME_POINTER_KEY` excluded on the strength of a name the ABI gave, deleting RBP-object recovery under frame-pointer omission (pinned both ways in `structs.test.ts`). `StructRegistry.provenanceCounts()` is a new instrument, reported beside `offsetof`.
- **The width refusal, found by the end-to-end test and not by design.** `inferTypes`' cast rule (`cast(var)` → `setType(var, castType)`) has only a narrow cast as evidence — a full-width `var` read carries none — so while the binding still produced casts, `mov [rdx], rcx / mov eax, ecx` printed `uint32_t arg_0` above `*(int64_t*)(arg_1) = arg_0;`. The var-at-width spelling removed the binding's casts, but a `movzx eax, cl` still lifts one, so the guard stays: `promoteVars` refuses an `intN_t` narrower than the register for a register parameter the body reads at full width (`fullWidthVarReads`: a `var` mentioned other than as a cast's direct operand), keeps it for a parameter read only narrowly (`uint8_t arg_0`), and WRITES the declared type back into `typeCtx` — `emit.ts` suppresses a cast equal to the operand's known type, and with header and context disagreeing `return (uint32_t)arg_0;` printed bare.
- **What moved, at base `098cb04` (`s29-params-base-098cb04` → this change).** staleReads `sites` 34/229/227/34 → 32/40/40/32 (t32/t64/w64/w32), `entryBound` 0 → 2/189/187/2, `copies` 8/156/147/8 → 6/0/0/6 — every x64 entry copy was an argument register's, which is why the liveness half is now `copies + entryBound > 0`; `wrong` and `copiesCorrupted` 0 throughout. Struct provenance links/views per binary t32 16/19 → 16/19, t64 21/39 → 23/36, w64 22/37 → 24/34, w32 16/18 → 16/18 (the base measured with the same accessor patched into a `098cb04` worktree, label `s29-params-base-prov`); the 10 t64 views withdrawn are all structs on a REUSED rcx that the register-name branch attributed to parameter 0 — `sub_14000B000`'s section-header pointer (`rcx = rax + r8 + 0x18`), `sub_140007EA0`/memset's running destination, `sub_140005CBC`'s loop pointer, read by hand — 4 honest views gained (`0x1400043dc:0`, `0x140005980:0`, `0x140007d00:1`, `0x14000f7e0:1`) and 2 caller links. `offsetof` 284/284 → 288/288 fields and 54 → 55 definitions on t64, 291/291 → 300/300 and 54 → 56 on w64, t32/w32 unmoved, ratio 1.00 throughout; `structOverlaps` 0; `paramClobberedAtEntry` 0; `unencodableNames` 0; `undeclaredIdentifiers` register+minted 0; `selfAssigns` wrong/unresolved 0. `signatureAgreement` x64 `over` 0 → 0, `under` 8 → 6, `agree` 271 → 273 / 267 → 269 — the six deduped homed slots per binary, which is also the whole of the compare's `declared params scanned 475 → 469 / 454 → 448 <<<` row; x86 `under` 0 → 26 on both t32 and w32, the arg_ecx/arg_edx count, reported not gated. Emitted C moved in 209/279 (t64), 207/275 (w64), 26/260 (t32), 26/258 (w32); guards CHANGED 128/123 on x64 with operator, verdict and edge unchanged on every one (ONE anchor A→A2 per binary, `edx == 0` → `arg_1 == 0` in `sub_140006AAC`/`sub_1400024B4`), 2 per x86 binary; 16/14 jccs entered the audited population and 2/1 left it — the leavers are guards now spelled through a `struct_15` field, whose `->` defeats the single-operator parse (the compare's `guards w/o single compare 188 → 195 / 165 → 171 <<<` row: an audit blind spot to struct-field spelling, not a change in what the C tests; 0 shared jccs lost a single top operator); polarity 0 inverted; emitted lines 17882 → 17445 / 16199 → 15777 (156/147 entry copies and the `int64_t rcx;`-class declarations gone, ~47 `rcx = arg_0` phi copies per binary added); `insns covered` 6099 → 6092 / 5525 → 5518; `unknown type names` 281 → 336 / 278 → 330 (more pointer-typed parameters reaching gcc's prelude, liveness only). Hand-read: 12 functions read against `objdump -d -M intel`: t64 `sub_140002100`, `sub_140006AAC`, `sub_14000B000`, `sub_1400080E0`, `sub_14000B094`, `sub_140007EA0`, `sub_140004D0C`, `sub_140005CBC`, `sub_1400045DC`; w64 `sub_14000BB30`; t32 `sub_40250D`, `sub_40CB8B` — every `arg_<n>`/`arg_ecx` an entry value, every header arity the register scan's, and the one wrong-value class found (`(uint32_t)arg_0 < 0`, 10 per x64 binary at the first run) fixed before landing. The identity deletion fires at **0 corpus sites**: every homed spill is `mov [rsp+N], ecx` and promotes as an `rsp`-keyed local (`var_8 = (uint32_t)arg_0` in `sub_1400080E0`), so its corpus control is INERT — recorded, and the rule rests on `promote.test.ts` and `pipeline.test.ts`.
