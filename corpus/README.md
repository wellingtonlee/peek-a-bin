...
| `readers` / `readerShapes` / `readerNamed` / `readerEmitted` | **The same question of every OTHER in-block flag reader** — `setcc` and `cmovcc` — since `peek-a-bin-n9cl.6` generalised the lifter's capture from the trailing Jcc to all of them (`operandCaptures`). `readers` is the population, `readerShapes` the spoiled ones (machine shape, the liveness half: **3 per binary** at `6299113`), `readerNamed` the emitted lines at a spoiled reader that do not read the capture — **the defect, gated at 0** — and `readerEmitted` the recovery. The emitted text judged is the line the line map places at the reader's own address; a reader the emitter admitted as `__unrecovered_N` or left `unlifted` is a refusal and is not judged. Disabling the capture for `setcc` takes `readerNamed` to the shape count. It is a LOWER bound for the Jcc rows' reason: a `setcc` whose assignment was folded into a later statement has no line at its address. |
