...
- `disasm/arm64*.ts` — ARM64 support: `arm64.ts` (fixed-width linear sweep at 4-byte alignment plus evidence-only function starts, split into `sweepArm64` + `decorateArm64Sweep` behind the unchanged `disassembleArm64`, plus `Arm64SweepCache` — see the gotcha), `arm64Operands.ts` (**the single A64 branch/address grammar** — the `ripRelative.ts` precedent, do not hand-roll a second one), `arm64Xref.ts` (`buildArm64Xrefs`). Everything x86-shaped — the decompiler, x86 xref building, IRP dispatch detection, stack frames, signatures — declines on ARM64 rather than guessing
...
- **Emitted call arity is checked against `apitypes.ts`'s declared signatures**, which is the only oracle in the repo that can see arity at all: `corpus/arity.ts`, **79/105, 90/127, 96/133 and 85/111 calls at the declared arity** (t32/t64/w64/w32, measured on base `91cca4f` with `peek-a-bin-6lmh` applied), with **under 26/34/34/26** — of which 0/26/26/0 sit at the ABI evidence's ceiling, where `collectArgs64`'s four fastcall registers cannot reach a five-parameter API — and **over 0/3/3/0**. Over-count is the direction that means an argument was *invented*, and every one of the 6 remaining rows is provably wrong (`GetLastError(rcx)`, `LeaveCriticalSection(…)`; the `GetModuleHandleW("KERNEL32.DLL", edi)` shape is retired), so **read it as `polarity inverted`'s character, not a baseline's** — it is nevertheless **reported, never gated**, because the count is not zero and a gate would have to pin today's absolute; a rise is judged in `compare.mjs`. `gcc -std=gnu89` accepts an implicit declaration at any arity and the emitter writes no prototypes, so the 842/842 figure **cannot move** on an arity defect. Validated by negative control: reverting `peek-a-bin-qb2x` (`collectArgs64` probing width-exactly again) takes t64 88→62 and w64 92→66 exact with over unmoved at 3, and `5 ExitProcess` reappears among the under rows (`peek-a-bin-02fa`; the instrument this replaces was lost with a deleted worktree, which is why these numbers are the first reproducible ones).
  ...
  **The x86 over-count is now 0 and the whole of the residual is x64.** It was 8/3/3/10 at `e22ba6e`; the x86 half was **two shapes, not one**, and both are now fixed — Shape 1, a nested call in argument position, by `peek-a-bin-f51x`, and Shape 2, the backwards walk running past the real arguments into the function's own callee-saved *prologue* pushes, by `peek-a-bin-6lmh` (see the two `collectArgs32` gotchas below). Shape 2 took t32 4→0 and w32 6→0 over, exact 75→79/105 and 79→85/111, **under unmoved at 26 on both**, x64 byte-identical.

  **`corpus/README.md`'s recorded upgrade — "if the OVER count is ever driven to 0, make it a gate at 0" — therefore did NOT trigger, and no threshold was invented at 3.** Six rows remain, 3 on each x64 binary (`2 GetLastError`, `1 LeaveCriticalSection`), and they are a `collectArgs64` defect, i.e. a different idea again: the x64 path reads arity out of `RegState` rather than walking pushes, so nothing in either x86 fix reaches them.
...
**Two of the defects `peek-a-bin-c33`'s forward flag model was expected to fix as a side effect are now fixed without it** (`peek-a-bin-jitf`, `peek-a-bin-xe01`), by two local refusals in `extractCondition` sharing `flagResult.ts`'s tables. That removes them from c33's justification and from what its Stage 3a has to demonstrate, and it means the **58 wrong-operand guards are already gone from the baseline** any future c33 measurement is taken against — so a c33 run that shows no movement there is agreeing with the current tree, not failing. It does not touch c33's own case: a guard's registers are still not IR readers, so SSA and dataflow still cannot see or repair one, and the ~300 stale unrepairable reads per binary are unchanged. What is now *unavailable* as evidence for c33 is "the forward model fixes jitf/xe01"; what remains is the structural argument.
...
- **A guard is answered from the flags the Jcc *actually* reads, and only while the compare still describes them.** Two refusals in `extractCondition`, one defect class — the emitted guard naming the **right operator over the wrong operands**, which no gate could see because polarity checks the operator, gcc compiles it, and it is not `__unrecovered_N`. (a) The forward walk clears the flags on any instruction that is not `isFlagTransparent`, because it used to clear them on *nothing* — not a call, not arithmetic, not a shift — so `cmp eax, 5 / … / sub ecx, edx / jne` emitted `eax != 5` where the machine branches on `ecx - edx != 0` (`peek-a-bin-jitf`; 2 corpus occurrences, both MSVC's branchless `cmp / sbb eax,eax / inc eax / jne` size check, where the **`inc`** sets the ZF the `jne` reads). (b) A compare whose operand something overwrote before the Jcc is refused, because the block's statements are emitted **above** the `if`: `cmp eax, 5 / mov eax, edx / je` printed `eax = edx;` and then `if (eax != 5)`, reading the new EAX (`peek-a-bin-xe01`). **Both use `flagResult.ts`, not a copy of it** — `isFlagTransparent` is that module's own table exported, and the overwrite scan is **one** `clobberedAfter` asked by `flagResultSetter`'s condition 6 of its *result* register and by `extractCondition` of the `cmp`'s *operands*. Those two paths in the same function had disagreed about whether a later write mattered since `peek-a-bin-b531`, with condition 6's docstring stating the `cmp` path's bug verbatim. **Refusal is the whole repair**: the guard becomes `__unrecovered_N`, which is "clean is not recovered" working as intended, and it is why the cost shows up as unrecovered values rising (108/85/67/85 → 159/94/73/122) rather than as guard text changing — `compare.mjs` reports `CHANGED 0` and `only-change 0` on all four binaries. Gated by `corpus/staleGuards.ts`; do not weaken either refusal without reading it.
