...
- **The emitted C is checked by a compiler.** `gcc -std=gnu89 -fsyntax-only` over every emitted function of the three x86 binaries: **all of them compile clean**, most recently **842 of 842** at `57f8406`. The denominator moved from 1001 because the detector was reporting every hot-patched function twice — MSVC's `8b ff 55 8b ec` prologue table has entries that are prefixes of one another, so t32 went 447 → 293 functions and 154 empty decompiled bodies → 0. Anyone pinning 447 or 1001 is pinning the defect (`peek-a-bin-dot`, `peek-a-bin-abv`). **t32 has been 288, not 293, since `95229d3`** — five MSVC `__finally` funclet bodies interior to their parents were correctly folded into them; measured, see `peek-a-bin-gtlf`.
