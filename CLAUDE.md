...
- **Emitted call arity is checked against `apitypes.ts`'s declared signatures**, which is the only oracle in the repo that can see arity at all: `corpus/arity.ts`, **70/105, 88/127, 92/133 and 73/111 calls at the declared arity** (t32/t64/w64/w32, measured at `e22ba6e`), with **under 27/36/38/28** — of which 0/26/26/0 sit at the ABI evidence's ceiling, where `collectArgs64`'s four fastcall registers cannot reach a five-parameter API — and **over 8/3/3/10**. Over-count is the direction that means an argument was *invented*, and every one of the 24 rows is provably wrong (`GetLastError(rcx)`, `GetProcessHeap(8, 0x1000)`), so **read it as `polarity inverted`'s character, not a baseline's** — it is nevertheless **reported, never gated**, because the count is not zero and a gate would have to pin today's absolute; a rise is judged in `compare.mjs`, and if over ever reaches 0 it should become a gate at 0. `gcc -std=gnu89` accepts an implicit declaration at any arity and the emitter writes no prototypes, so the 842/842 figure **cannot move** on an arity defect. Validated by negative control: reverting `peek-a-bin-qb2x` (`collectArgs64` probing width-exactly again) takes t64 88→62 and w64 92→66 exact with over unmoved at 3, and `5 ExitProcess` reappears among the under rows (`peek-a-bin-02fa`; the instrument this replaces was lost with a deleted worktree, which is why the numbers above are the first reproducible ones).
- **A register read that names a value it does not hold is a gate**: `corpus/staleReads.ts`, **0 wrong and 0 spoiled repairs on all four binaries**, from 28/78/28/78 wrong reads and 13/19/13/18 spoiled copies before the fix. Unlike the two audits above this one *is* gated at zero — every row it can report is a provably wrong name (a store through the wrong pointer, a call given the wrong argument), which is `polarity inverted`'s character rather than a count awaiting a threshold. It must run `foldBlock`: stopping at `destroySSA` reports 151 on t64 instead of 78, by counting dominating writes that fold into their single use (`peek-a-bin-dqpk`).
