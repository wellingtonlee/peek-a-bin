...
- **Condition polarity is audited per guard against the originating jcc**, not spot-checked: **1276 guards, 0 inverted**, and every subsequent structuring change re-ran the audit. It now covers all four emitted shapes — `if`, `while`, `for` and `do/while` — and resolves candidates through `jmp`-only blocks, since anchoring to the wrong jcc one edge away produced a false inverted on an ordinary MSVC loop entry (`peek-a-bin-8r0`, `peek-a-bin-lbz`).
- **A loop being told about every way the machine can leave it** is a separate audit from polarity, and it found what polarity cannot: 319 innermost loops matched to machine loops, of which exactly **one** offered fewer exits than the machine had — a `do { … } while (1)`, an unconditional call inside a loop the reader is told never ends. Not the structurer: its missing tests jump past the end of the *detected* function, and `buildCFG` draws no edge to a target outside the range it was given, so the block reads as unconditional. Such an arm is now emitted with a comment, since the transfer has no label to `goto` and no function to call; the standing fix is function sizing and belongs to `functionDetect.ts` (37 conditional jumps corpus-wide are in this position — `peek-a-bin-lbz`, `peek-a-bin-g7yp`).
- **The emitted C is checked by a compiler.** `gcc -std=gnu89 -fsyntax-only` over every emitted function of the three x86 binaries: **847 of 847 compile clean**. The denominator moved from 1001 because the detector was reporting every hot-patched function twice — MSVC's `8b ff 55 8b ec` prologue table has entries that are prefixes of one another, so t32 went 447 → 293 functions and 154 empty decompiled bodies → 0. Anyone pinning 447 or 1001 is pinning the defect (`peek-a-bin-dot`, `peek-a-bin-abv`).
- **Struct layouts are verified by a compiled and *run* `offsetof` program**, not by reading the declaration: 197/197 definitions and 1007/1007 fields lay out at the offsets their field names record, from 18/149 and 318/852 (`peek-a-bin-ey0`).
...
**"Clean" is not "recovered."** 287 of those 847 functions contain an *admitted* gap — a
...
behaviour, not a defect count, but do not read "847/847 compile" as "847 functions are right".
...
| `emit.ts` | `emitExpr` / `emitStmt`, plus `liveInStmt` and `collectAssignedRegs` — the backward liveness and the assigned-register set behind a call's result assignment. Both are full `IRStmt` switches, so a new statement kind breaks the build in four places in this file |
